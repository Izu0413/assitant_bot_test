...
def _search_applicable(select: dict) -> bool:
    """探索を適用する選択かどうか(タスク2: MAINに加えCTX_TO_ACTIVEへ拡張)。

    CTX_TO_ACTIVE = 自分の昇格(倒された後の後続)と Boss's Orders の標的の両方。
    候補は最大でもベンチ枚数程度なのでプレフィルタ不要。"""
    if select["type"] == SELECT_MAIN:
        return True
    if select["type"] == SELECT_CARD and select["context"] == CTX_TO_ACTIVE:
        return True
    return False
